      ...
      >
        {cards.map((card, index) => (
          <SwiperSlide key={index}>
            <div className="p-1.5 rounded-[28px] bg-black/[0.04] backdrop-blur-md flex flex-col justify-between h-[480px]">
              {/* Premium super-translucent white glass container (bg-white/25) for high elite appeal */}
              <div className="bg-white/25 backdrop-blur-md rounded-[23px] p-8 flex flex-col justify-between h-full">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[11px] font-bold text-black uppercase tracking-[0.08em] opacity-80">
                      {card.title}
                    </span>
                  </div>
                  ...
                  <div className="mt-[24px] text-left">
                    <span className="font-sans font-normal text-[60px] md:text-[68px] lg:text-[76px] tracking-[-0.04em] text-black leading-none block">
                      {card.value}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-4">
                  {card.details.map((detail, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-[11px] text-black/60 font-medium">
                      <span className="w-1.5 h-1.5 rounded-full bg-black/30 mt-1 shrink-0" />
                      <span>{detail}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-3 pb-2.5 px-6">
                <span className="font-mono text-[10px] font-medium text-black/55 uppercase tracking-widest block truncate">
                  {card.footer}
                </span>
              </div>
            </div>
